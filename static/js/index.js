  ...
  view: function(vnode) {
    return m('.app', !vnode.state.chosenUser ? null : [
      m(heroListUserView, {
        ...
        removeSuperHero: (relPk, idx) => {vnode.state.chosenUser.removeSuperHero(relPk, idx)},
        addSuperHero: (universe, pk) => {vnode.state.chosenUser.requestRandomSuperHeroes(universe, pk)},
        saveSuperHeroSelection: ops => {vnode.state.chosenUser.saveSuperHeroSelection(ops)},
        cancelSuperHeroSelection: () => {vnode.state.chosenUser.cancelSuperHeroSelection()}
      })
    ]);
  }
};
